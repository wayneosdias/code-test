...
#include "cyapicallbacks.h"
...
#if (UART_DEB_UART_WAKE_ENABLE_CONST && UART_DEB_UART_RX_WAKEUP_IRQ)
    /**
    * \addtogroup group_globals
    * \{
    */
    /** This global variable determines whether to enable Skip Start
    * functionality when UART_DEB_Sleep() function is called:
    * 0 – disable, other values – enable. Default value is 1.
    * It is only available when Enable wakeup from Deep Sleep Mode is enabled.
    */
    uint8 UART_DEB_skipStart = 1u;
    /** \} globals */
#endif /* (UART_DEB_UART_WAKE_ENABLE_CONST && UART_DEB_UART_RX_WAKEUP_IRQ) */
    ...
    ****************************************************************************//**
    *
    *  Configures the UART_DEB for UART operation.
    *
    *  This function is intended specifically to be used when the UART_DEB
    *  configuration is set to “Unconfigured UART_DEB” in the customizer.
    *  After initializing the UART_DEB in UART mode using this function,
    *  the component can be enabled using the UART_DEB_Start() or
    * UART_DEB_Enable() function.
...
/*******************************************************************************
* Function Name: UART_DEB_UartPostEnable
****************************************************************************//**
*
...
void UART_DEB_UartPostEnable(void)
{
        ...
        if (UART_DEB_CHECK_TX_SCL_MISO_PIN_USED)
        {
            /* Set SCB UART to drive the output pin */
            UART_DEB_SET_HSIOM_SEL(UART_DEB_TX_SCL_MISO_HSIOM_REG, UART_DEB_TX_SCL_MISO_HSIOM_MASK,
                                           UART_DEB_TX_SCL_MISO_HSIOM_POS, UART_DEB_TX_SCL_MISO_HSIOM_SEL_UART);
        }
    #endif /* (UART_DEB_TX_SCL_MISO_PIN_PIN) */

    #if !(UART_DEB_CY_SCBIP_V0 || UART_DEB_CY_SCBIP_V1)
        #if (UART_DEB_RTS_SS0_PIN)
            if (UART_DEB_CHECK_RTS_SS0_PIN_USED)
            {
                /* Set SCB UART to drive the output pin */
                UART_DEB_SET_HSIOM_SEL(UART_DEB_RTS_SS0_HSIOM_REG, UART_DEB_RTS_SS0_HSIOM_MASK,
                                               UART_DEB_RTS_SS0_HSIOM_POS, UART_DEB_RTS_SS0_HSIOM_SEL_UART);
            }
         ...
         /* Set SCB UART to drive the output pin */
        UART_DEB_SET_HSIOM_SEL(UART_DEB_TX_HSIOM_REG, UART_DEB_TX_HSIOM_MASK,
                                       UART_DEB_TX_HSIOM_POS, UART_DEB_TX_HSIOM_SEL_UART);
    #endif /* (UART_DEB_UART_TX_PIN) */

    #if (UART_DEB_UART_RTS_PIN)
        /* Set SCB UART to drive the output pin */
        UART_DEB_SET_HSIOM_SEL(UART_DEB_RTS_HSIOM_REG, UART_DEB_RTS_HSIOM_MASK,
                                       UART_DEB_RTS_HSIOM_POS, UART_DEB_RTS_HSIOM_SEL_UART);
    #endif /* (UART_DEB_UART_RTS_PIN) */
#endif /* (UART_DEB_SCB_MODE_UNCONFIG_CONST_CFG) */

    /* Restore TX interrupt sources. */
    UART_DEB_SetTxInterruptMode(UART_DEB_IntrTxMask);
}


/*******************************************************************************
* Function Name: UART_DEB_UartStop
****************************************************************************//**
*
...
void UART_DEB_UartStop(void)
{
        ...
        if (UART_DEB_CHECK_TX_SCL_MISO_PIN_USED)
        {
            ...
            UART_DEB_SET_HSIOM_SEL(UART_DEB_TX_SCL_MISO_HSIOM_REG, UART_DEB_TX_SCL_MISO_HSIOM_MASK,
                                           UART_DEB_TX_SCL_MISO_HSIOM_POS, UART_DEB_TX_SCL_MISO_HSIOM_SEL_GPIO);
        }
    #endif /* (UART_DEB_TX_SCL_MISO_PIN_PIN) */
            ...
            if (UART_DEB_CHECK_RTS_SS0_PIN_USED)
            {
                ...
                UART_DEB_uart_rts_spi_ss0_Write(UART_DEB_GET_UART_RTS_INACTIVE);

                /* Set GPIO to drive output pin */
                UART_DEB_SET_HSIOM_SEL(UART_DEB_RTS_SS0_HSIOM_REG, UART_DEB_RTS_SS0_HSIOM_MASK,
                                               UART_DEB_RTS_SS0_HSIOM_POS, UART_DEB_RTS_SS0_HSIOM_SEL_GPIO);
            }
        #endif /* (UART_DEB_RTS_SS0_PIN) */
    #endif /* !(UART_DEB_CY_SCBIP_V0 || UART_DEB_CY_SCBIP_V1) */
        ...
        /* Set GPIO to drive output pin */
        UART_DEB_SET_HSIOM_SEL(UART_DEB_TX_HSIOM_REG, UART_DEB_TX_HSIOM_MASK,
                                       UART_DEB_TX_HSIOM_POS, UART_DEB_TX_HSIOM_SEL_GPIO);
    #endif /* (UART_DEB_UART_TX_PIN) */

    #if (UART_DEB_UART_RTS_PIN)
        /* Set output pin state after block is disabled */
        UART_DEB_rts_Write(UART_DEB_GET_UART_RTS_INACTIVE);

        /* Set GPIO to drive output pin */
        UART_DEB_SET_HSIOM_SEL(UART_DEB_RTS_HSIOM_REG, UART_DEB_RTS_HSIOM_MASK,
                                       UART_DEB_RTS_HSIOM_POS, UART_DEB_RTS_HSIOM_SEL_GPIO);
    #endif /* (UART_DEB_UART_RTS_PIN) */
    ...
    UART_DEB_IntrTxMask = LO16(UART_DEB_GetTxInterruptMode() & UART_DEB_INTR_UART_TX_RESTORE);
}
...
void UART_DEB_UartSetRxAddressMask(uint32 addressMask)
{
    ...
    *   Check UART_DEB_rxBufferOverflow to capture that error condition.
    *
    *******************************************************************************/
    ...
    *******************************************************************************/
    void UART_DEB_UartSaveConfig(void)
    {
        ...
        */
        UART_DEB_CLEAR_UART_RX_WAKE_INTR;
    ...
    *******************************************************************************/
    void UART_DEB_UartRestoreConfig(void)
    {
        ...
        *******************************************************************************/
        ...
        {
        #ifdef UART_DEB_UART_WAKEUP_ISR_ENTRY_CALLBACK
            UART_DEB_UART_WAKEUP_ISR_EntryCallback();
        #endif /* UART_DEB_UART_WAKEUP_ISR_ENTRY_CALLBACK */

            UART_DEB_CLEAR_UART_RX_WAKE_INTR;

        #ifdef UART_DEB_UART_WAKEUP_ISR_EXIT_CALLBACK
            UART_DEB_UART_WAKEUP_ISR_ExitCallback();
        #endif /* UART_DEB_UART_WAKEUP_ISR_EXIT_CALLBACK */
        }
    #endif /* (UART_DEB_UART_RX_WAKEUP_IRQ) */
#endif /* (UART_DEB_UART_RX_WAKEUP_IRQ) */
